...
export function ImageRotate() {
  ...
  async function choose(next: File) {
    ...
    setFile(next); setBitmap(loaded); setAngle(0); setFlipX(false); setFlipY(false); setStatus(`${loaded.width} × ${loaded.height}`);
  }
  ...
  async function exportImage() {
    ...
    downloadBlob(blob, replaceExtension(file.name, `-rotated-${Math.round(angle)}`, format)); setStatus("New image downloaded");
  }
